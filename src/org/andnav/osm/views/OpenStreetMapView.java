...
public class OpenStreetMapView extends View implements OpenStreetMapConstants,
	...
	public OpenStreetMapView(final Context context, OpenStreetMapRendererInfo aRendererInfo) {
		super(context);
		if(OpenStreetMapViewConstants.DEBUGMODE) Log.d("RMAPS-Me-OSMView", "OpenStreetMapView Constructor is Called");
	...
	public Handler getHandler(){
		if(OpenStreetMapViewConstants.DEBUGMODE) Log.d("RMAPS-Me-OSMView-OSMTileProvider", "OSMView getHandler is Called");
...
//		}
	...
	private BoundingBoxE6 getBoundingBox(final int pViewWidth, final int pViewHeight){
		if(OpenStreetMapViewConstants.DEBUGMODE) Log.d("RMAPS-Me-OSMView", "OSMView getBoundingBox is called");
		if(OpenStreetMapViewConstants.DEBUGMODE) Log.d("RMAPS-Me-OSMView", "OSMView getBoundingBox about to call getMapTileFromCoordinates");
		...
		if(OpenStreetMapViewConstants.DEBUGMODE) Log.d("RMAPS-Me-OSMView", "OSMView getBoundingBox about to call getBoundingBoxFromMapTile");
	...
	public OpenStreetMapViewProjection getProjection() {
		if(OpenStreetMapViewConstants.DEBUGMODE) Log.d("RMAPS-Me-OSMView", "OSMViewProjection getProjection is called");
	...
	protected void setMapCenter(final int aLatitudeE6, final int aLongitudeE6,
		...
		if(OpenStreetMapViewConstants.DEBUGMODE) Log.d("RMAPS-Me-OSMView", "OSMView setMapCenter is called");
		...
		if(OpenStreetMapViewConstants.DEBUGMODE) Log.d("RMAPS-Me-OSMView", "OSMView setMapCenter about to call invalidate!");
	...
	public boolean setRenderer(final OpenStreetMapRendererInfo aRenderer) {
		...
		if(OpenStreetMapViewConstants.DEBUGMODE) Log.d("RMAPS-Me-OSMView", "OSMView setRenderer is Called and rabout to invalidate view by setting zoomlevel");
		
		this.setZoomLevel(this.mZoomLevel); // Invalidates the map and zooms to
											// the maximum level of the
											// renderer.
		
		if(OpenStreetMapViewConstants.DEBUGMODE) Log.d("RMAPS-Me-OSMView", "OpenStreetMapView setRenderer is Called and ret is set to value of mSimpleInvalidationHandler and ret is: "+ret);
		return ret;
	}
	...
	public void setZoomLevel(final int aZoomLevel) {
		if(OpenStreetMapViewConstants.DEBUGMODE) Log.d("RMAPS-Me-OSMView", "OpenStreetMapView setZoomLevel is Called and about to invalidate view");
...
//		this.mTileProvider.preCacheTile(nextBelowMaptileUrlString);
	...
	public void onLongPress(MotionEvent e) {
		...
		if(OpenStreetMapViewConstants.DEBUGMODE) Log.d("RMAPS-Me-OSMView", "OSMView SimpleInvalidationHandler message: " + callbackMessage);
	...
	public boolean onDoubleTap(MotionEvent e) {
		if (mBearing != 0) {
			mBearing = 0;
			if(OpenStreetMapViewConstants.DEBUGMODE) Log.d("RMAPS-Me-OSMView", "OSMView onDoubleTap give mMainActivityCallBackHandler the user_moved_map id: " + R.id.user_moved_map);
...
//		Log.e(DEBUGTAG, "onKeyDown keyCode="+keyCode);
	...
	private class GestureCallback implements VersionedGestureDetector.OnGestureListener {
		...
		public void onDown(MotionEvent event) {
			if(OpenStreetMapViewConstants.DEBUGMODE) Log.d("RMAPS-Me-OSMView", "OSMView onDown about to invalidate view");
		...
		public void onMove(MotionEvent event, int count, float x1, float y1, float x2, float y2) {
			if (Math.max(Math.abs(mTouchDownX - event.getX()), Math.abs(mTouchDownY - event.getY())) > 6 && !mStopMoveDetecting) {
				...
				if(OpenStreetMapViewConstants.DEBUGMODE) Log.d("RMAPS-Me-OSMView", "OSMView onMove about to invalidate view");
				invalidate();
				if(OpenStreetMapViewConstants.DEBUGMODE) Log.d("RMAPS-Me-OSMView", "OSMView onMove give mMainActivityCallBackHandler the user_moved_map id: " + R.id.user_moved_map);
		...
		public void onUp(MotionEvent event) {
			...
			if(OpenStreetMapViewConstants.DEBUGMODE) Log.d("RMAPS-Me-OSMView", "OSMView onUp about to invalidate view");
		...
		public void onUp2(MotionEvent event) {
			...
			if(OpenStreetMapViewConstants.DEBUGMODE) Log.d("RMAPS-Me-OSMView", "OSMView onUp2 give mMainActivityCallBackHandler set_title: " + R.id.set_title);
			Message.obtain(mMainActivityCallbackHandler, R.id.set_title).sendToTarget();
		}
	...
	public void onDraw(final Canvas c) {
		if(OpenStreetMapViewConstants.DEBUGMODE) Log.d("RMAPS-Me-OSMView", "OpenStreetMapView onDraw is Called");
		/*
		...
		for (int y = -additionalTilesNeededToTopOfCenter; y <= additionalTilesNeededToBottomOfCenter; y++) {
			for (int x = -additionalTilesNeededToLeftOfCenter; x <= additionalTilesNeededToRightOfCenter; x++) {
				...
				if(OpenStreetMapViewConstants.DEBUGMODE) Log.d("RMAPS-Me-OSMView", "OSMView onDraw Loop is about to call getTileURLString and y is: "+y + " and x is:" + x);
...
//				Ut.dd("onDraw: " + tileURLString);
				...
				if(OpenStreetMapViewConstants.DEBUGMODE) Log.d("RMAPS-Me-OSMView", "OSMView onDraw Loop about to call getMapTile with tileURLString: " + tileURLString);
				if(OpenStreetMapViewConstants.DEBUGMODE) Log.d("RMAPS-Me-OSMView", "OSMView onDraw Loop SimpleInvalidationHandler message: " + callbackMessage);
				...
				if (currentMapTile != null){
					if(OpenStreetMapViewConstants.DEBUGMODE) Log.d("RMAPS-Me-OSMView", "OSMView onDraw Loop getMapTile is NOT NULL and titleURLString is: " + tileURLString);
					if(OpenStreetMapViewConstants.DEBUGMODE) Log.d("RMAPS-Me-OSMView", "OSMView onDraw Loop SimpleInvalidationHandler message: " + callbackMessage);
					...
					if (DEBUGMODE)
					{
						Log.d("RMAPS-Me-OSMView", "OSMView.onDraw Loop getMapTile is not null and titleURLString is: " + tileURLString);
				...
				//louis is adding a default image for the map here...
				//else currentMapTile = this.mTileProvider.getMapTile(

			}
		}
		if(OpenStreetMapViewConstants.DEBUGMODE) Log.d("RMAPS-Me-OSMView", "OSMView onDraw out of Loop SimpleInvalidationHandler message: " + callbackMessage);
		
		if(OpenStreetMapViewConstants.DEBUGMODE) Log.d("RMAPS-Me-OSMView", "OSMView.onDraw out of Loop, now about to call OSMTileProvider to call OSMTileCache to Commit");
		this.mTileProvider.CommitCash();
		
		if(OpenStreetMapViewConstants.DEBUGMODE) Log.d("RMAPS-Me-OSMView", "OSMView.onDraw out of Loop, now about to draw Overlays...");
		/* Draw all Overlays. */
		for (OpenStreetMapViewOverlay osmvo : this.mOverlays)
		...
		c.restore();
		if(OpenStreetMapViewConstants.DEBUGMODE) Log.d("RMAPS-Me-OSMView", "OSMView.onDraw has called restore and now method is over...");
		if(OpenStreetMapViewConstants.DEBUGMODE) Log.d("RMAPS-Me-OSMView", "OSMView SimpleInvalidationHandler message: " + callbackMessage);
		//louis.fazen@gmail.com is adding a crazy line
		Message.obtain(mSimpleInvalidationHandler, OpenStreetMapTileFilesystemProvider.MAPTILEFSLOADER_SUCCESS_ID + 13).sendToTarget();
		if(OpenStreetMapViewConstants.DEBUGMODE) Log.d("RMAPS-Me-OSMView", "OSMView SimpleInvalidationHandler message: " + callbackMessage);
...
//		c.drawLine(viewWidth/2+100, viewHeight/2-100, viewWidth/2-100, viewHeight/2+100, this.mPaint);
	...
	public class OpenStreetMapViewProjection {
		...
		public OpenStreetMapViewProjection() {
			if(OpenStreetMapViewConstants.DEBUGMODE) Log.d("RMAPS-Me-OSMView", "OSMView OSMViewProjection is Called");
			 ...
			 * this.mLongitudeE6 .
			 */
			if(OpenStreetMapViewConstants.DEBUGMODE) Log.d("RMAPS-Me-OSMView", "OSMView OSMViewProjection is about to call getMapTileFromCoordinates");
			...
			upperLeftCornerOfCenterMapTile = getUpperLeftCornerOfCenterMapTileInScreen(
					centerMapTileCoords, tileSizePx, null);
			if(OpenStreetMapViewConstants.DEBUGMODE) Log.d("RMAPS-Me-OSMView", "OSMView OSMViewProjection is about to call getBoundingBoxE6");
	...
	private class SimpleInvalidationHandler extends Handler {
		...
		public void handleMessage(final Message msg) {
			//louis.fazen@gmail.com added this part
			callbackMessage = msg.what;
			if(OpenStreetMapViewConstants.DEBUGMODE) Log.d("RMAPS-Me-OSMView", "OSMView SimpleInvalidationHandler is sent message and the  message is: " + callbackMessage);
			...
			case OpenStreetMapTileFilesystemProvider.MAPTILEFSLOADER_SUCCESS_ID:
				...
				if(OpenStreetMapViewConstants.DEBUGMODE) Log.d("RMAPS-Me-OSMView", "OSMView SimpleInvalidationHandler has invalidated OSMView with message: " + callbackMessage);
			...
			case OpenStreetMapTileFilesystemProvider.ERROR_MESSAGE:
				...
				if(OpenStreetMapViewConstants.DEBUGMODE) Log.d("RMAPS-Me-OSMView", "OSMView SimpleInvalidationhandler set MainActivityCallbackHandler to an error message");
			...
			case OpenStreetMapTileFilesystemProvider.INDEXIND_SUCCESS_ID:
				...
				Message.obtain(mMainActivityCallbackHandler, R.id.set_title).sendToTarget();
				if(OpenStreetMapViewConstants.DEBUGMODE) Log.d("RMAPS-Me-OSMView", "OSMView SimpleInvalidationhandler set MainActivity to INDEXIND_SUCCESS_ID with R.id.set_title: " + R.id.set_title);
				
				if(OpenStreetMapViewConstants.DEBUGMODE) Log.d("RMAPS-Me-OSMView", "OSMView SimpleInvalidationhandler invalidate OSMView");
				OpenStreetMapView.this.invalidate();
				break;
			}
		}
	...
	public void setMainActivityCallbackHandler(Handler callbackHandler) {
		...
		if(OpenStreetMapViewConstants.DEBUGMODE) Log.d("RMAPS-Me-OSMView", "OSMView SetMainActivityCallbackHandler");
