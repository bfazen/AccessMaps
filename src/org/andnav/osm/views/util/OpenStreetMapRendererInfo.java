...
public class OpenStreetMapRendererInfo {
	...
	public OpenStreetMapRendererInfo(Resources aRes, String aId){
		mResources = aRes;
		if(OpenStreetMapViewConstants.DEBUGMODE) Log.d("RMAPS-Me", "RendererInfo Constructor is called and aId is: "+ aId);
		mLastUpdateTime = 0L;
		mTimeStamp = "";
		this.ID = "Region_Mapnik_sqlitedb";
		this.NAME = "Region.Mapnik.sqlitedb";
		this.BASEURL = "/sdcard/rmaps/maps/Region.Mapnik.sqlitedb";
		this.ZOOM_MINLEVEL = 1;
		this.ZOOM_MAXLEVEL = 19;
		this.IMAGE_FILENAMEENDING = "sqlitedb";
		this.MAPTILE_SIZEPX = 256;
		this.URL_BUILDER_TYPE = 1;
		//louis.fazen wants to change this tile_source_type to 5 for sqlitedb!
		...
		if(OpenStreetMapViewConstants.DEBUGMODE) Log.d("RMAPS-Me", "Calls RendererInfo Constructor is called and ID is: "+ this.ID);
		if(OpenStreetMapViewConstants.DEBUGMODE) Log.d("RMAPS-Me", "Calls RendererInfo Constructor is called and NAME is: "+ this.NAME);
		if(OpenStreetMapViewConstants.DEBUGMODE) Log.d("RMAPS-Me", "Calls RendererInfo Constructor is called and BASEURL is: "+ this.BASEURL);
		if(OpenStreetMapViewConstants.DEBUGMODE) Log.d("RMAPS-Me", "Calls RendererInfo Constructor is called and ZOOM_MINLEVEL is: "+ this.ZOOM_MINLEVEL);
		if(OpenStreetMapViewConstants.DEBUGMODE) Log.d("RMAPS-Me", "Calls RendererInfo Constructor is called and ZOOM_MAXLEVEL is: "+ this.ZOOM_MAXLEVEL);
		if(OpenStreetMapViewConstants.DEBUGMODE) Log.d("RMAPS-Me", "Calls RendererInfo Constructor is called and MAPTILESIZEPX is: "+ this.MAPTILE_SIZEPX);
		if(OpenStreetMapViewConstants.DEBUGMODE) Log.d("RMAPS-Me", "Calls RendererInfo Constructor is called and URL_BUILDER_TYPE is: "+ this.URL_BUILDER_TYPE);
		
	}

	public void LoadFromResources(String aId, SharedPreferences pref) {
		if(OpenStreetMapViewConstants.DEBUGMODE) Log.d("RMAPS-Me", "RendererInfo LoadFromResources is called and aId is: "+ aId);
		if (aId.equalsIgnoreCase(""))
			aId = "mapnik";
		// aId = "usermap_Region_Mapnik_sqlitedb";

		if(pref != null)
			//louis.fazen@gmail.com changed boolean prefs for online cache
			mOnlineMapCacheEnabled = pref.getBoolean("pref_onlinecache", false);
		else
		...
		if (aId.contains("usermap_")) {
			...
			if(OpenStreetMapViewConstants.DEBUGMODE) Log.d("RMAPS-Me", "Calls RendererInfo LoadFromResources is called and ID is: "+ this.ID);
			if(OpenStreetMapViewConstants.DEBUGMODE) Log.d("RMAPS-Me", "Calls RendererInfo LoadFromResources is called and NAME is: "+ this.NAME);
			if(OpenStreetMapViewConstants.DEBUGMODE) Log.d("RMAPS-Me", "Calls RendererInfo LoadFromResources is called and BASEURL is: "+ this.BASEURL);
			if(OpenStreetMapViewConstants.DEBUGMODE) Log.d("RMAPS-Me", "Calls RendererInfo LoadFromResources is called and ZOOM_MINLEVEL is: "+ this.ZOOM_MINLEVEL);
			if(OpenStreetMapViewConstants.DEBUGMODE) Log.d("RMAPS-Me", "Calls RendererInfo LoadFromResources is called and ZOOM_MAXLEVEL is: "+ this.ZOOM_MAXLEVEL);
			if(OpenStreetMapViewConstants.DEBUGMODE) Log.d("RMAPS-Me", "Calls RendererInfo LoadFromResources is called and MAPTILESIZEPX is: "+ this.MAPTILE_SIZEPX);
			if(OpenStreetMapViewConstants.DEBUGMODE) Log.d("RMAPS-Me", "Calls RendererInfo LoadFromResources is called and URL_BUILDER_TYPE is: "+ this.URL_BUILDER_TYPE);
			
			if (aId.toLowerCase().endsWith("sqlitedb")) {
				this.TILE_SOURCE_TYPE = 5;
				this.IMAGE_FILENAMEENDING = "";
				 if(OpenStreetMapViewConstants.DEBUGMODE) Log.d("RMAPS-Me", "sqlitedb is recognized and this.IMAGE_FILENAMEENDING is: "+ this.IMAGE_FILENAMEENDING);
			}
	...
	public String getTileURLString(final int[] tileID, final int zoomLevel){
		if(OpenStreetMapViewConstants.DEBUGMODE) Log.d("RMAPS-Me", "RendererInfo getTileURLString is called");
		...
		case 4: // TAR file
			...
			.toString();
		default:
			if(OpenStreetMapViewConstants.DEBUGMODE) Log.d("RMAPS-Me", "RendererInfo getTileURLString SQLiteDB string is called");
			return new StringBuilder().append(this.BASEURL)
			.append("/")
			.append(zoomLevel)
			.append("/")
