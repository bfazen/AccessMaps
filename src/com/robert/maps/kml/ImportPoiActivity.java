...
*/
	...
	public void loadPOI(){
		...
		defaultKml = "/sdcard/rmaps/import/default.kml";
		if(OpenStreetMapViewConstants.DEBUGMODE) Log.d("RMAPS-Me-LoadPoi", "loadPoi Called and defaultKml now set to: " + defaultKml);
		defaultPOI =  true;
		doImportPOI();
	}
	private void doImportPOI() {
		if(OpenStreetMapViewConstants.DEBUGMODE) Log.d("RMAPS-Me-LoadPoi", "doImportPoi Called");
		if(OpenStreetMapViewConstants.DEBUGMODE) Log.d("RMAPS-Me-LoadPoi", "importPoiFilename not yet set: " + importPoiFilename);
		importPoiFilename = mFileName.getText().toString();
		if (defaultPOI){
			if(OpenStreetMapViewConstants.DEBUGMODE) Log.d("RMAPS-Me-LoadPoi", "defaultPOI is true");
			importPoiFilename = defaultKml;
			defaultPOI = false;
		}
