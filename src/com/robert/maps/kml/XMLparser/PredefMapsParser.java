		...
		if(localName.equalsIgnoreCase(MAP)){
			if (mRendererInfo != null) {
				...
				if(i == -1 || !attributes.getValue(LAYER).equalsIgnoreCase(TRUE)){
					...
					//louis.fazen@gmail.com  this does set the pref on the pref page to not checked, but cancels out any effect of behavior (they still show up, and checking them changes nothing)
